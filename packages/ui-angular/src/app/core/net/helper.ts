...
export const REFRESH_HEADER_KEY = 'refresh-token';
 ...
 * - Attach auth & refresh headers in Delon-expected keys to avoid drift
 */
export function getAdditionalHeaders(headers?: HttpHeaders): Record<string, string> {
  ...
  const tokenSrv = inject(DA_SERVICE_TOKEN);
  const model = tokenSrv.get() as Record<string, any> | null;
  const accessToken = model?.token;
  if (accessToken && !headers?.has(AUTH_HEADER_KEY)) {
    res[AUTH_HEADER_KEY] = `Bearer ${accessToken}`;
  }

  const refreshToken = model?.refresh_token;
  if (refreshToken && !headers?.has(REFRESH_HEADER_KEY)) {
    res[REFRESH_HEADER_KEY] = refreshToken;
  }
