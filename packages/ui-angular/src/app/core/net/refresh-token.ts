...
import { REFRESH_HEADER_KEY, getAdditionalHeaders, toLogin } from './helper';
...
function reAttachToken(injector: Injector, req: HttpRequest<any>): HttpRequest<any> {
  const mergedHeaders = {
    ...getAdditionalHeaders(req.headers)
  };
  return req.clone({ setHeaders: mergedHeaders });
}
...
function refreshTokenRequest(injector: Injector): Observable<any> {
  ...
  return injector.get(HttpClient).post(`/api/auth/refresh`, {}, { headers: { [REFRESH_HEADER_KEY]: refreshToken } });
}
