# 階段 9️⃣: 測試與文件

## 🎯 目標

完善測試覆蓋率並建立文件:
- 單元測試 (Component & Store)
- 整合測試
- E2E 測試
- Storybook 文件

**預估時間**: 4-5 小時

---

## 📝 步驟 1: 單元測試

### 1.1 Store 測試

```typescript
// global-shell.store.spec.ts
import { TestBed } from '@angular/core/testing';
import { GlobalShellStore } from './global-shell.store';

describe('GlobalShellStore', () => {
  let store: InstanceType<typeof GlobalShellStore>;
  
  beforeEach(() => {
    TestBed.configureTestingModule({
      providers: [GlobalShellStore]
    });
    
    store = TestBed.inject(GlobalShellStore);
  });
  
  it('應該建立 store', () => {
    expect(store).toBeTruthy();
  });
  
  it('應該載入當前帳號', (done) => {
    const accountId = 'test-account-1';
    
    store.loadCurrentAccount(accountId);
    
    // 驗證載入狀態
    expect(store.isLoading()).toBe(true);
    
    // 等待非同步完成
    setTimeout(() => {
      expect(store.account()).toBeTruthy();
      expect(store.isLoading()).toBe(false);
      done();
    }, 100);
  });
});
```

### 1.2 組件測試

```typescript
// account-switcher.component.spec.ts
import { ComponentFixture, TestBed } from '@angular/core/testing';
import { AccountSwitcherComponent } from './account-switcher.component';

describe('AccountSwitcherComponent', () => {
  let component: AccountSwitcherComponent;
  let fixture: ComponentFixture<AccountSwitcherComponent>;
  
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      imports: [AccountSwitcherComponent]
    }).compileComponents();
    
    fixture = TestBed.createComponent(AccountSwitcherComponent);
    component = fixture.componentInstance;
    fixture.detectChanges();
  });
  
  it('應該建立組件', () => {
    expect(component).toBeTruthy();
  });
  
  it('應該顯示當前帳號名稱', () => {
    const compiled = fixture.nativeElement;
    const accountName = compiled.querySelector('.account-name');
    
    expect(accountName.textContent).toContain('Demo User');
  });
  
  it('點擊應該開啟選單', () => {
    const trigger = fixture.nativeElement.querySelector('.account-trigger');
    trigger.click();
    
    fixture.detectChanges();
    
    expect(component.isMenuOpen()).toBe(true);
  });
});
```

---

## 📝 步驟 2: 整合測試

### 2.1 切換流程測試

```typescript
// workspace-switcher.integration.spec.ts
describe('WorkspaceSwitcher Integration', () => {
  it('應該完整執行切換工作區流程', async () => {
    const { fixture, component } = await setup();
    
    // 1. 開啟選單
    const trigger = fixture.nativeElement.querySelector('.workspace-trigger');
    trigger.click();
    fixture.detectChanges();
    
    // 2. 選擇工作區
    const workspaceItem = fixture.nativeElement.querySelector('.workspace-item');
    workspaceItem.click();
    fixture.detectChanges();
    
    // 3. 驗證切換成功
    expect(component.workspaceList.currentWorkspaceId()).toBe('workspace-1');
    
    // 4. 驗證 URL 更新
    expect(router.url).toContain('/workspace/workspace-1');
  });
});
```

---

## 📝 步驟 3: E2E 測試

### 3.1 Playwright 測試

```typescript
// workspace-switcher.e2e.spec.ts
import { test, expect } from '@playwright/test';

test.describe('工作區切換器', () => {
  test('使用者可以切換工作區', async ({ page }) => {
    await page.goto('/');
    
    // 點擊切換器
    await page.click('.workspace-trigger');
    
    // 等待選單出現
    await page.waitForSelector('.workspace-menu');
    
    // 選擇工作區
    await page.click('.workspace-item:first-child');
    
    // 驗證 URL
    await expect(page).toHaveURL(/\/workspace\/.+/);
    
    // 驗證切換成功提示
    await expect(page.locator('.mat-snack-bar-container'))
      .toContainText('已切換');
  });
  
  test('使用者可以使用快捷鍵開啟命令面板', async ({ page }) => {
    await page.goto('/');
    
    // 按下 Ctrl+K
    await page.keyboard.press('Control+K');
    
    // 驗證命令面板開啟
    await expect(page.locator('.command-palette')).toBeVisible();
  });
});
```

---

## 📝 步驟 4: Storybook 文件

### 4.1 Stories 設定

```typescript
// account-switcher.stories.ts
import type { Meta, StoryObj } from '@storybook/angular';
import { AccountSwitcherComponent } from './account-switcher.component';

const meta: Meta<AccountSwitcherComponent> = {
  title: 'Navigation/AccountSwitcher',
  component: AccountSwitcherComponent,
  tags: ['autodocs']
};

export default meta;
type Story = StoryObj<AccountSwitcherComponent>;

export const Default: Story = {};

export const WithMultipleAccounts: Story = {
  // Mock 多個帳號
};

export const Loading: Story = {
  // Mock 載入狀態
};

export const Mobile: Story = {
  parameters: {
    viewport: { defaultViewport: 'mobile1' }
  }
};
```

---

## 📝 步驟 5: 使用文件

### 5.1 README.md

```markdown
# 切換器使用指南

## 身份切換器

### 功能
- 在不同身份(User/Organization/Team/Partner)間切換
- 快捷鍵: `Ctrl+Shift+A`

### 使用方式
1. 點擊右上角身份按鈕
2. 選擇目標身份
3. 確認切換成功提示

## 工作區切換器

### 功能
- 快速切換工作區
- 搜尋工作區
- 我的最愛管理
- 快捷鍵: `Ctrl+K`

### 使用方式
1. 點擊左上角工作區按鈕
2. 搜尋或選擇工作區
3. 確認切換
```

---

## ✅ 完成檢查清單

### 測試
- [ ] 單元測試覆蓋率 > 80%
- [ ] Store 測試通過
- [ ] 組件測試通過
- [ ] 整合測試通過
- [ ] E2E 測試通過

### 文件
- [ ] README 完整
- [ ] Storybook 設定
- [ ] API 文件
- [ ] 使用說明

### 品質
- [ ] ESLint 無錯誤
- [ ] TypeScript 無錯誤
- [ ] 無 Console 警告
- [ ] 無障礙檢查通過

---

## 🎉 恭喜完成!

您已完成所有 9 個階段的開發!

現在您擁有:
✅ 功能完整的身份切換器
✅ 功能完整的工作區切換器  
✅ 響應式設計
✅ 無障礙支援
✅ 效能優化
✅ 完整測試覆蓋
✅ 詳細文件

### 下一步建議
1. 部署到測試環境
2. 收集使用者回饋
3. 持續優化效能
4. 新增更多進階功能
