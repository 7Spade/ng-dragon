# 階段 3️⃣: 身份切換器 - 互動與狀態

## 🎯 目標

實作身份切換器的互動邏輯:
- 載入所有可切換的身份列表
- 實作切換帳號功能
- 處理載入狀態
- 錯誤處理與提示
- 成功回饋

**預估時間**: 2-3 小時
**依賴**: 階段 2 (身份切換器 - 基礎組件)

---

## 📝 步驟 1: 擴展 Store 功能

### 1.1 更新 GlobalShellStore

```typescript
// src/app/core/stores/global-shell.store.ts (新增方法)
import { MatSnackBar } from '@angular/material/snack-bar';

export const GlobalShellStore = signalStore(
  { providedIn: 'root' },
  
  withState(initialState),
  
  withComputed((store) => ({
    // ... 現有的 computed
    
    /**
     * 所有可用帳號列表
     */
    availableAccounts: computed(() => store.accounts()),
    
    /**
     * 按類型分組的帳號
     */
    groupedAccounts: computed(() => {
      const accounts = store.accounts();
      
      return {
        user: accounts.filter(a => a.type === AccountType.User),
        organizations: accounts.filter(a => a.type === AccountType.Organization),
        teams: accounts.filter(a => a.type === AccountType.Team),
        partners: accounts.filter(a => a.type === AccountType.Partner)
      };
    })
  })),
  
  withMethods((store) => {
    const accountService = inject(AccountService);
    const snackBar = inject(MatSnackBar);
    
    return {
      // ... 現有方法
      
      /**
       * 載入所有可用帳號
       */
      loadAvailableAccounts: rxMethod<string>(
        pipe(
          switchMap((userId) =>
            accountService.getUserAccounts(userId).pipe(
              tap((accounts) => {
                patchState(store, { accounts });
              }),
              catchError((error) => {
                console.error('載入帳號列表失敗:', error);
                return of([]);
              })
            )
          )
        )
      ),
      
      /**
       * 切換帳號 (帶有 UI 回饋)
       */
      switchAccountWithFeedback: rxMethod<string>(
        pipe(
          tap(() => {
            patchState(store, { isLoading: true, error: null });
          }),
          switchMap((accountId) =>
            accountService.switchAccount(accountId).pipe(
              tap((account) => {
                patchState(store, {
                  account,
                  isLoading: false,
                  error: null
                });
                
                // 顯示成功訊息
                snackBar.open(
                  `已切換至 ${account.name}`,
                  '關閉',
                  { duration: 3000 }
                );
              }),
              catchError((error) => {
                patchState(store, {
                  isLoading: false,
                  error: error.message
                });
                
                // 顯示錯誤訊息
                snackBar.open(
                  '切換帳號失敗,請稍後再試',
                  '關閉',
                  { duration: 5000 }
                );
                
                return of(null);
              })
            )
          )
        )
      )
    };
  })
);
```

### 1.2 更新 State 介面

```typescript
// src/app/core/models/account.model.ts (更新)
export interface CurrentAccountState {
  account: Account | null;
  accounts: Account[]; // 新增:所有可用帳號
  membership?: AccountMembership;
  isLoading: boolean;
  error: string | null;
}
```

---

## 📝 步驟 2: 更新組件邏輯

### 2.1 擴展 AccountSwitcherComponent

```typescript
// src/app/layout/header/account-switcher/account-switcher.component.ts
import { Component, computed, inject, signal, OnInit } from '@angular/core';
import { MatSnackBar, MatSnackBarModule } from '@angular/material/snack-bar';

@Component({
  selector: 'app-account-switcher',
  standalone: true,
  imports: [
    // ... 現有 imports
    MatSnackBarModule
  ],
  templateUrl: './account-switcher.component.html',
  styleUrl: './account-switcher.component.scss'
})
export class AccountSwitcherComponent implements OnInit {
  protected globalShell = inject(GlobalShellStore);
  protected snackBar = inject(MatSnackBar);
  
  protected isMenuOpen = signal(false);
  protected readonly AccountType = AccountType;
  
  // Computed: 分組帳號
  protected groupedAccounts = this.globalShell.groupedAccounts;
  
  // Computed: 當前帳號 ID
  protected currentAccountId = computed(() => {
    return this.globalShell.account()?.id;
  });
  
  ngOnInit(): void {
    // 載入所有可用帳號
    // TODO: 從 Auth 取得當前 userId
    const userId = 'current-user-id';
    this.globalShell.loadAvailableAccounts(userId);
  }
  
  /**
   * 切換帳號
   */
  onSwitchAccount(accountId: string): void {
    // 如果是當前帳號,不執行切換
    if (accountId === this.currentAccountId()) {
      return;
    }
    
    // 執行切換
    this.globalShell.switchAccountWithFeedback(accountId);
  }
  
  /**
   * 檢查是否為當前帳號
   */
  isCurrentAccount(accountId: string): boolean {
    return accountId === this.currentAccountId();
  }
  
  /**
   * 開啟管理帳號頁面
   */
  onManageAccounts(): void {
    // TODO: 導航到管理帳號頁面
    console.log('開啟管理帳號頁面');
  }
  
  // ... 其他現有方法
}
```

---

## 📝 步驟 3: 更新模板

### 3.1 動態渲染帳號列表

```html
<!-- account-switcher.component.html (更新) -->

<mat-menu
  #accountMenu="matMenu"
  class="account-menu"
  xPosition="before"
  yPosition="below">
  
  <!-- 個人帳號 -->
  <div class="menu-section">
    <div class="section-title">個人帳號</div>
    
    @if (groupedAccounts().user.length > 0) {
      @for (account of groupedAccounts().user; track account.id) {
        <button
          mat-menu-item
          class="account-item"
          [class.active]="isCurrentAccount(account.id)"
          (click)="onSwitchAccount(account.id)"
          [disabled]="globalShell.isLoading()">
          
          <div class="account-item-content">
            <div class="item-avatar">
              @if (account.photoURL) {
                <img [src]="account.photoURL" [alt]="account.name">
              } @else {
                <mat-icon>person</mat-icon>
              }
            </div>
            
            <div class="item-info">
              <div class="item-name">{{ account.displayName }}</div>
              <div class="item-meta">User</div>
            </div>
            
            @if (isCurrentAccount(account.id)) {
              <mat-icon class="check-icon">check</mat-icon>
            }
          </div>
        </button>
      }
    } @else {
      <div class="empty-state">無可用帳號</div>
    }
  </div>
  
  <mat-divider></mat-divider>
  
  <!-- 組織 -->
  <div class="menu-section">
    <div class="section-title">組織</div>
    
    @if (groupedAccounts().organizations.length > 0) {
      @for (org of groupedAccounts().organizations; track org.id) {
        <button
          mat-menu-item
          class="account-item"
          [class.active]="isCurrentAccount(org.id)"
          (click)="onSwitchAccount(org.id)"
          [disabled]="globalShell.isLoading()">
          
          <div class="account-item-content">
            <div class="item-avatar">
              @if (org.orgLogo) {
                <img [src]="org.orgLogo" [alt]="org.name">
              } @else {
                <mat-icon>business</mat-icon>
              }
            </div>
            
            <div class="item-info">
              <div class="item-name">{{ org.name }}</div>
              <div class="item-meta">
                Organization · {{ org.memberCount }} members
              </div>
            </div>
            
            @if (isCurrentAccount(org.id)) {
              <mat-icon class="check-icon">check</mat-icon>
            }
          </div>
        </button>
      }
    } @else {
      <div class="empty-state">尚未加入任何組織</div>
    }
  </div>
  
  <mat-divider></mat-divider>
  
  <!-- 團隊 -->
  <div class="menu-section">
    <div class="section-title">團隊</div>
    
    @if (groupedAccounts().teams.length > 0) {
      @for (team of groupedAccounts().teams; track team.id) {
        <button
          mat-menu-item
          class="account-item"
          [class.active]="isCurrentAccount(team.id)"
          (click)="onSwitchAccount(team.id)"
          [disabled]="globalShell.isLoading()">
          
          <div class="account-item-content">
            <div class="item-avatar">
              <mat-icon>groups</mat-icon>
            </div>
            
            <div class="item-info">
              <div class="item-name">{{ team.name }}</div>
              <div class="item-meta">
                Team · {{ team.memberCount }} members
              </div>
            </div>
            
            @if (isCurrentAccount(team.id)) {
              <mat-icon class="check-icon">check</mat-icon>
            }
          </div>
        </button>
      }
    } @else {
      <div class="empty-state">尚未加入任何團隊</div>
    }
  </div>
  
  <mat-divider></mat-divider>
  
  <!-- 合作夥伴 -->
  <div class="menu-section">
    <div class="section-title">合作夥伴</div>
    
    @if (groupedAccounts().partners.length > 0) {
      @for (partner of groupedAccounts().partners; track partner.id) {
        <button
          mat-menu-item
          class="account-item"
          [class.active]="isCurrentAccount(partner.id)"
          (click)="onSwitchAccount(partner.id)"
          [disabled]="globalShell.isLoading()">
          
          <div class="account-item-content">
            <div class="item-avatar">
              <mat-icon>handshake</mat-icon>
            </div>
            
            <div class="item-info">
              <div class="item-name">{{ partner.companyName }}</div>
              <div class="item-meta">
                Partner · {{ partner.partnerLevel }}
              </div>
            </div>
            
            @if (isCurrentAccount(partner.id)) {
              <mat-icon class="check-icon">check</mat-icon>
            }
          </div>
        </button>
      }
    } @else {
      <div class="empty-state">尚未建立合作夥伴關係</div>
    }
  </div>
  
  <mat-divider></mat-divider>
  
  <!-- 管理帳號 -->
  <button
    mat-menu-item
    class="manage-accounts"
    (click)="onManageAccounts()">
    <mat-icon>settings</mat-icon>
    <span>管理帳號...</span>
  </button>
</mat-menu>
```

---

## 📝 步驟 4: 載入狀態處理

### 4.1 禁用狀態樣式

```scss
// account-switcher.component.scss (新增)

.account-item {
  // ... 現有樣式
  
  // 禁用狀態
  &[disabled] {
    opacity: 0.5;
    cursor: not-allowed;
    
    &:hover {
      background: transparent !important;
    }
  }
}

// 載入中的選單遮罩 (可選)
::ng-deep .account-menu {
  &.loading {
    &::after {
      content: '';
      position: absolute;
      top: 0;
      left: 0;
      right: 0;
      bottom: 0;
      background: rgba(255, 255, 255, 0.7);
      display: flex;
      align-items: center;
      justify-content: center;
      z-index: 10;
    }
  }
}
```

---

## 📝 步驟 5: 快捷鍵支援

### 5.1 新增快捷鍵服務

```typescript
// src/app/core/services/keyboard-shortcut.service.ts
import { Injectable, inject } from '@angular/core';
import { fromEvent, filter, map } from 'rxjs';
import { DOCUMENT } from '@angular/common';

export interface KeyboardShortcut {
  key: string;
  ctrl?: boolean;
  shift?: boolean;
  alt?: boolean;
  meta?: boolean;
}

@Injectable({
  providedIn: 'root'
})
export class KeyboardShortcutService {
  private document = inject(DOCUMENT);
  
  /**
   * 註冊快捷鍵
   */
  register(shortcut: KeyboardShortcut) {
    return fromEvent<KeyboardEvent>(this.document, 'keydown').pipe(
      filter((event) => this.matchesShortcut(event, shortcut)),
      map((event) => {
        event.preventDefault();
        return event;
      })
    );
  }
  
  private matchesShortcut(
    event: KeyboardEvent,
    shortcut: KeyboardShortcut
  ): boolean {
    const keyMatch = event.key.toLowerCase() === shortcut.key.toLowerCase();
    const ctrlMatch = shortcut.ctrl ? event.ctrlKey || event.metaKey : true;
    const shiftMatch = shortcut.shift ? event.shiftKey : true;
    const altMatch = shortcut.alt ? event.altKey : true;
    
    return keyMatch && ctrlMatch && shiftMatch && altMatch;
  }
}
```

### 5.2 在組件中使用快捷鍵

```typescript
// account-switcher.component.ts (新增)
import { takeUntilDestroyed } from '@angular/core/rxjs-interop';
import { KeyboardShortcutService } from '@/core/services/keyboard-shortcut.service';

export class AccountSwitcherComponent implements OnInit {
  private keyboardShortcut = inject(KeyboardShortcutService);
  private destroyRef = inject(DestroyRef);
  
  ngOnInit(): void {
    // ... 現有初始化
    
    // 註冊 Ctrl+Shift+A 快捷鍵
    this.keyboardShortcut
      .register({ key: 'a', ctrl: true, shift: true })
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe(() => {
        // 觸發選單開啟
        // 需要透過 ViewChild 取得 MatMenuTrigger
        this.menuTrigger?.openMenu();
      });
  }
}
```

---

## ✅ 驗證清單

- [ ] 點擊帳號項目可成功切換
- [ ] 切換中顯示載入指示器
- [ ] 切換成功顯示成功訊息
- [ ] 切換失敗顯示錯誤訊息
- [ ] 當前帳號有勾選標記
- [ ] 空狀態正確顯示
- [ ] 快捷鍵 Ctrl+Shift+A 可開啟選單
- [ ] 切換後選單自動關閉

---

## 🐛 常見問題

### Q: MatSnackBar 不顯示?
**A**: 確認已在 `app.config.ts` 中提供 MatSnackBar

### Q: 快捷鍵不生效?
**A**: 檢查是否有其他全域快捷鍵衝突

---

## ⏭️ 下一步

完成後繼續 **階段 4: 工作區切換器 - 基礎組件**
