# 階段 2️⃣: 身份切換器 - 基礎組件

## 🎯 目標

實作身份切換器的核心 UI 組件:
- 觸發按鈕 (Trigger Button)
- 下拉選單 (MatMenu)
- 選單項目樣式
- 基本的視覺設計

**預估時間**: 3-4 小時
**依賴**: 階段 1 (基礎架構設置)

---

## 📁 檔案結構

```
src/app/
├── layout/
│   └── header/
│       ├── account-switcher/
│       │   ├── account-switcher.component.ts
│       │   ├── account-switcher.component.html
│       │   ├── account-switcher.component.scss
│       │   └── account-switcher.component.spec.ts
│       └── header.component.ts
```

---

## 📝 步驟 1: 建立組件

### 1.1 產生組件

```bash
# 使用 Angular CLI 產生組件
ng generate component layout/header/account-switcher --standalone
```

### 1.2 組件 TypeScript (`account-switcher.component.ts`)

```typescript
// src/app/layout/header/account-switcher/account-switcher.component.ts
import { Component, computed, inject, signal } from '@angular/core';
import { CommonModule } from '@angular/common';
import { MatButtonModule } from '@angular/material/button';
import { MatMenuModule } from '@angular/material/menu';
import { MatIconModule } from '@angular/material/icon';
import { MatDividerModule } from '@angular/material/divider';
import { MatProgressSpinnerModule } from '@angular/material/progress-spinner';

import { GlobalShellStore } from '@/core/stores';
import { AccountType } from '@/core/models';

@Component({
  selector: 'app-account-switcher',
  standalone: true,
  imports: [
    CommonModule,
    MatButtonModule,
    MatMenuModule,
    MatIconModule,
    MatDividerModule,
    MatProgressSpinnerModule
  ],
  templateUrl: './account-switcher.component.html',
  styleUrl: './account-switcher.component.scss'
})
export class AccountSwitcherComponent {
  // Stores
  protected globalShell = inject(GlobalShellStore);
  
  // 選單狀態
  protected isMenuOpen = signal(false);
  
  // 公開 AccountType 給模板使用
  protected readonly AccountType = AccountType;
  
  // Computed values
  protected displayName = computed(() => {
    const account = this.globalShell.account();
    if (!account) return '未登入';
    
    // 根據帳號類型取得名稱
    if (account.type === AccountType.User) {
      return account.displayName || account.name;
    }
    return account.name;
  });
  
  protected avatarUrl = computed(() => {
    return this.globalShell.accountAvatar();
  });
  
  protected accountType = computed(() => {
    return this.globalShell.account()?.type;
  });
  
  /**
   * 選單開啟
   */
  onMenuOpened(): void {
    this.isMenuOpen.set(true);
  }
  
  /**
   * 選單關閉
   */
  onMenuClosed(): void {
    this.isMenuOpen.set(false);
  }
  
  /**
   * 取得帳號類型圖示
   */
  getAccountTypeIcon(type: AccountType): string {
    switch (type) {
      case AccountType.User:
        return 'person';
      case AccountType.Organization:
        return 'business';
      case AccountType.Team:
        return 'groups';
      case AccountType.Partner:
        return 'handshake';
      default:
        return 'person';
    }
  }
  
  /**
   * 取得帳號類型徽章顏色
   */
  getBadgeColor(type: AccountType): string {
    switch (type) {
      case AccountType.Organization:
        return 'primary';
      case AccountType.Team:
        return 'accent';
      case AccountType.Partner:
        return 'warn';
      default:
        return '';
    }
  }
}
```

---

## 📝 步驟 2: 建立模板

### 2.1 HTML 模板 (`account-switcher.component.html`)

```html
<!-- src/app/layout/header/account-switcher/account-switcher.component.html -->

<!-- 觸發按鈕 -->
<button
  mat-button
  class="account-trigger"
  [matMenuTriggerFor]="accountMenu"
  (menuOpened)="onMenuOpened()"
  (menuClosed)="onMenuClosed()"
  aria-label="切換身份"
  [attr.aria-expanded]="isMenuOpen()">
  
  <!-- 頭像區域 -->
  <div class="avatar-container">
    @if (avatarUrl(); as avatar) {
      <img
        [src]="avatar"
        [alt]="displayName()"
        class="avatar"
        loading="lazy">
    } @else {
      <mat-icon class="avatar-icon">
        {{ getAccountTypeIcon(accountType() || AccountType.User) }}
      </mat-icon>
    }
    
    <!-- 身份類型徽章 -->
    @if (accountType() && accountType() !== AccountType.User) {
      <span
        class="badge"
        [attr.data-type]="accountType()">
      </span>
    }
  </div>
  
  <!-- 帳號名稱 -->
  <span class="account-name">{{ displayName() }}</span>
  
  <!-- 下拉箭頭 -->
  @if (!globalShell.isLoading()) {
    <mat-icon
      class="dropdown-icon"
      [class.rotated]="isMenuOpen()">
      expand_more
    </mat-icon>
  } @else {
    <mat-spinner diameter="16" class="loading-spinner"></mat-spinner>
  }
</button>

<!-- 下拉選單 -->
<mat-menu
  #accountMenu="matMenu"
  class="account-menu"
  xPosition="before"
  yPosition="below">
  
  <!-- 個人帳號區塊 -->
  <div class="menu-section">
    <div class="section-title">個人帳號</div>
    
    <button
      mat-menu-item
      class="account-item"
      [class.active]="accountType() === AccountType.User">
      
      <div class="account-item-content">
        <!-- 左側:頭像 -->
        <div class="item-avatar">
          @if (avatarUrl(); as avatar) {
            <img [src]="avatar" [alt]="displayName()">
          } @else {
            <mat-icon>person</mat-icon>
          }
        </div>
        
        <!-- 中間:資訊 -->
        <div class="item-info">
          <div class="item-name">{{ displayName() }}</div>
          <div class="item-meta">User</div>
        </div>
        
        <!-- 右側:勾選圖示 -->
        @if (accountType() === AccountType.User) {
          <mat-icon class="check-icon">check</mat-icon>
        }
      </div>
    </button>
  </div>
  
  <mat-divider></mat-divider>
  
  <!-- 組織區塊 -->
  <div class="menu-section">
    <div class="section-title">組織</div>
    
    <!-- TODO: 這裡將從 Store 取得組織列表 -->
    <div class="empty-state">
      尚未加入任何組織
    </div>
  </div>
  
  <mat-divider></mat-divider>
  
  <!-- 團隊區塊 -->
  <div class="menu-section">
    <div class="section-title">團隊</div>
    
    <div class="empty-state">
      尚未加入任何團隊
    </div>
  </div>
  
  <mat-divider></mat-divider>
  
  <!-- 合作夥伴區塊 -->
  <div class="menu-section">
    <div class="section-title">合作夥伴</div>
    
    <div class="empty-state">
      尚未建立合作夥伴關係
    </div>
  </div>
  
  <mat-divider></mat-divider>
  
  <!-- 管理帳號選項 -->
  <button mat-menu-item class="manage-accounts">
    <mat-icon>settings</mat-icon>
    <span>管理帳號...</span>
  </button>
</mat-menu>
```

---

## 📝 步驟 3: 建立樣式

### 3.1 SCSS 樣式 (`account-switcher.component.scss`)

```scss
// src/app/layout/header/account-switcher/account-switcher.component.scss

// 觸發按鈕
.account-trigger {
  display: flex;
  align-items: center;
  gap: 8px;
  height: 48px;
  padding: 8px 12px;
  border-radius: 24px;
  background: transparent;
  transition: background-color 200ms ease;
  
  &:hover {
    background: rgba(0, 0, 0, 0.04);
  }
  
  &:active {
    background: rgba(0, 0, 0, 0.08);
  }
  
  // 頭像容器
  .avatar-container {
    position: relative;
    width: 32px;
    height: 32px;
    flex-shrink: 0;
    
    // 頭像圖片
    .avatar {
      width: 100%;
      height: 100%;
      border-radius: 50%;
      object-fit: cover;
    }
    
    // 頭像圖示 (備用)
    .avatar-icon {
      width: 100%;
      height: 100%;
      font-size: 24px;
      display: flex;
      align-items: center;
      justify-content: center;
      background: rgba(0, 0, 0, 0.12);
      border-radius: 50%;
      color: rgba(0, 0, 0, 0.6);
    }
    
    // 身份類型徽章
    .badge {
      position: absolute;
      bottom: 0;
      right: 0;
      width: 8px;
      height: 8px;
      border-radius: 50%;
      border: 2px solid white;
      
      &[data-type="organization"] {
        background: #1976d2; // 藍色
      }
      
      &[data-type="team"] {
        background: #ff9800; // 橙色
      }
      
      &[data-type="partner"] {
        background: #4caf50; // 綠色
      }
    }
  }
  
  // 帳號名稱
  .account-name {
    font-size: 14px;
    font-weight: 500;
    color: rgba(0, 0, 0, 0.87);
    max-width: 120px;
    overflow: hidden;
    text-overflow: ellipsis;
    white-space: nowrap;
  }
  
  // 下拉箭頭
  .dropdown-icon {
    width: 20px;
    height: 20px;
    font-size: 20px;
    transition: transform 200ms ease;
    
    &.rotated {
      transform: rotate(180deg);
    }
  }
  
  // 載入中指示器
  .loading-spinner {
    ::ng-deep .mat-mdc-progress-spinner circle {
      stroke: rgba(0, 0, 0, 0.6);
    }
  }
}

// 下拉選單
::ng-deep .account-menu {
  .mat-mdc-menu-panel {
    width: 280px;
    max-height: 400px;
    border-radius: 8px;
  }
  
  .mat-mdc-menu-content {
    padding: 8px 0;
  }
}

// 選單區塊
.menu-section {
  padding: 0;
  
  // 區塊標題
  .section-title {
    font-size: 12px;
    font-weight: 500;
    color: rgba(0, 0, 0, 0.6);
    text-transform: uppercase;
    letter-spacing: 0.5px;
    padding: 8px 16px;
    height: 32px;
    line-height: 16px;
  }
  
  // 空狀態
  .empty-state {
    font-size: 12px;
    color: rgba(0, 0, 0, 0.38);
    padding: 12px 16px;
    font-style: italic;
  }
}

// 帳號選項
.account-item {
  height: 56px !important;
  padding: 8px 16px !important;
  
  // 當前選中
  &.active {
    background: rgba(63, 81, 181, 0.08);
    
    .item-name {
      font-weight: 500;
    }
  }
  
  // 懸停效果
  &:hover:not(.active) {
    background: rgba(0, 0, 0, 0.04);
  }
  
  .account-item-content {
    display: flex;
    align-items: center;
    gap: 12px;
    width: 100%;
    
    // 頭像
    .item-avatar {
      width: 32px;
      height: 32px;
      flex-shrink: 0;
      
      img {
        width: 100%;
        height: 100%;
        border-radius: 50%;
        object-fit: cover;
      }
      
      mat-icon {
        width: 100%;
        height: 100%;
        font-size: 24px;
        display: flex;
        align-items: center;
        justify-content: center;
        background: rgba(0, 0, 0, 0.12);
        border-radius: 50%;
        color: rgba(0, 0, 0, 0.6);
      }
    }
    
    // 資訊區
    .item-info {
      flex: 1;
      min-width: 0;
      
      .item-name {
        font-size: 14px;
        font-weight: 400;
        color: rgba(0, 0, 0, 0.87);
        overflow: hidden;
        text-overflow: ellipsis;
        white-space: nowrap;
      }
      
      .item-meta {
        font-size: 12px;
        color: rgba(0, 0, 0, 0.6);
        margin-top: 2px;
      }
    }
    
    // 勾選圖示
    .check-icon {
      width: 20px;
      height: 20px;
      font-size: 20px;
      color: #3f51b5; // Primary color
      flex-shrink: 0;
    }
  }
}

// 管理帳號按鈕
.manage-accounts {
  display: flex;
  align-items: center;
  gap: 12px;
  height: 48px !important;
  
  mat-icon {
    color: rgba(0, 0, 0, 0.6);
  }
  
  span {
    font-size: 14px;
    color: rgba(0, 0, 0, 0.87);
  }
}

// 分隔線
::ng-deep .account-menu mat-divider {
  margin: 8px 0;
}
```

---

## 📝 步驟 4: 整合到 Header

### 4.1 更新 Header 組件

```typescript
// src/app/layout/header/header.component.ts
import { Component } from '@angular/core';
import { CommonModule } from '@angular/common';
import { MatToolbarModule } from '@angular/material/toolbar';

import { AccountSwitcherComponent } from './account-switcher/account-switcher.component';

@Component({
  selector: 'app-header',
  standalone: true,
  imports: [
    CommonModule,
    MatToolbarModule,
    AccountSwitcherComponent
  ],
  template: `
    <mat-toolbar color="primary" class="app-header">
      <div class="header-container">
        <!-- Logo -->
        <div class="logo">
          <span>My App</span>
        </div>
        
        <!-- Spacer -->
        <span class="spacer"></span>
        
        <!-- 身份切換器 -->
        <app-account-switcher />
      </div>
    </mat-toolbar>
  `,
  styles: [`
    .app-header {
      position: sticky;
      top: 0;
      z-index: 1000;
    }
    
    .header-container {
      display: flex;
      align-items: center;
      width: 100%;
      padding: 0 16px;
    }
    
    .logo {
      font-size: 20px;
      font-weight: 600;
    }
    
    .spacer {
      flex: 1;
    }
  `]
})
export class HeaderComponent {}
```

---

## 📝 步驟 5: 初始化 Store

### 5.1 在 App Component 初始化

```typescript
// src/app/app.component.ts
import { Component, OnInit, inject } from '@angular/core';
import { RouterOutlet } from '@angular/router';

import { GlobalShellStore } from '@/core/stores';
import { HeaderComponent } from './layout/header/header.component';

@Component({
  selector: 'app-root',
  standalone: true,
  imports: [
    RouterOutlet,
    HeaderComponent
  ],
  template: `
    <app-header />
    <main class="app-main">
      <router-outlet />
    </main>
  `,
  styles: [`
    :host {
      display: flex;
      flex-direction: column;
      height: 100vh;
    }
    
    .app-main {
      flex: 1;
      overflow: auto;
    }
  `]
})
export class AppComponent implements OnInit {
  private globalShell = inject(GlobalShellStore);
  
  ngOnInit(): void {
    // TODO: 從 Firebase Auth 取得當前使用者
    // 暫時使用模擬資料
    const mockAccount = {
      id: 'user-1',
      type: 'user' as const,
      name: 'Demo User',
      email: 'demo@example.com',
      displayName: 'Demo User',
      photoURL: 'https://i.pravatar.cc/150?img=1',
      createdAt: new Date(),
      updatedAt: new Date()
    };
    
    // 載入當前帳號 (暫時直接設置)
    // this.globalShell.loadCurrentAccount('user-1');
  }
}
```

---

## ✅ 驗證清單

完成此階段後,確認以下項目:

- [ ] AccountSwitcherComponent 可正常編譯
- [ ] 觸發按鈕正確顯示頭像和名稱
- [ ] 點擊按鈕可開啟下拉選單
- [ ] 下拉選單顯示正確的分組結構
- [ ] 當前帳號有勾選標記
- [ ] 空狀態文字正確顯示
- [ ] 樣式符合設計規範
- [ ] 響應式按鈕互動正常 (hover, active)

---

## 🎨 視覺檢查

在瀏覽器中檢查:

1. **觸發按鈕**
   - 頭像圓形,大小 32x32px
   - 名稱文字 14px,可截斷
   - 下拉箭頭可旋轉
   - Hover 效果正常

2. **下拉選單**
   - 寬度 280px
   - 圓角 8px
   - 陰影正確
   - 位置對齊右側

3. **選單項目**
   - 高度 56px
   - 分組標題小寫,灰色
   - 空狀態斜體,淺灰色
   - 勾選圖示藍色

---

## 🐛 常見問題

### Q: 下拉箭頭沒有旋轉動畫?
**A**: 確認 SCSS 中的 `transition` 屬性和 `[class.rotated]` 綁定正確

### Q: Material 組件樣式錯亂?
**A**: 確認在 `angular.json` 中已引入 Material 主題:
```json
"styles": [
  "@angular/material/prebuilt-themes/indigo-pink.css",
  "src/styles.scss"
]
```

### Q: 圖片無法顯示?
**A**: 檢查 CSP 政策或使用本地圖片資源

---

## ⏭️ 下一步

完成此階段後,請繼續 **階段 3: 身份切換器 - 互動與狀態** (`03-身份切換器-互動與狀態.md`)
